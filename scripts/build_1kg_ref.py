...
This is a one-time script to generate the reference frequency file used by
``popout label``. It downloads (or reads local) 1000 Genomes Phase 3 VCFs
and computes per-superpopulation allele frequencies for biallelic SNPs.

Output format (gzipped TSV):
    #chrom  pos  ref  alt  EUR  EAS  AMR  AFR  SAS

Usage:
    python scripts/build_1kg_ref.py --vcf-dir /path/to/1kg/vcfs --out 1kg_superpop_freq.GRCh38.tsv.gz

    Or with a single multi-chromosome VCF:
    python scripts/build_1kg_ref.py --vcf /path/to/ALL.chr*.vcf.gz --out 1kg_superpop_freq.GRCh38.tsv.gz

Requirements:
    - pysam
    - 1KG Phase 3 sample panel file (downloaded automatically or via --panel)
"""
